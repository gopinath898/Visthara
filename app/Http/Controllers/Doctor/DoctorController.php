...
class DoctorController extends Controller
{
    ...
    public function doctorSignup()
    {
        ...
        return view('doctor.auth.doctor_register',compact('countries','categories'));
    }
    ...
    public function verify_doctor(Request $request)
    {
        ...
        if(Auth::attempt(['email' => request('email'), 'password' => request('password')]))
        {
            ...
            {
                Auth::logout();
                return redirect()->back()->withErrors('Therapist can login');
            }
        }
        else
        {
    ...
    public function doctor_home()
    {
        ...
        $doctor = Doctor::where('user_id',auth()->user()->id)->first();
        $today_Appointments = Appointment::whereDate('created_at',Carbon::now(env('timezone')))->where('payment_status',1)->where('doctor_id',$doctor->id)->orderBy('id','DESC')->get();
        ...
        $totalUser = AppointmentSessions::leftjoin('appointment','appointment.appointment_id','appointment_sessions.appointment_id')->where('appointment_sessions.doctor_id',$doctor->id)->groupBy('appointment.user_id')->count();

        $totalAppointment = AppointmentSessions::where('doctor_id',$doctor->id)->count();
        $listappointment = Appointment::where('doctor_id',$doctor->id)->where('payment_status',1)->get();
        $totalReview = Review::where('doctor_id',$doctor->id)->count();
        return view('doctor.doctor.home',compact('today_Appointments','allUsers','totalReview','totalAppointment','totalUser','orderCharts','currency','listappointment'));
    }
    ...
    public function schedule()
    {
        ...
        $doctor->workingHours = WorkingHour::where('doctor_id',$doctor->id)->whereDate('booking_date', '>=', date('Y-m-d'))->get();
        $doctor->firstHours = WorkingHour::where('doctor_id',$doctor->id)->whereDate('booking_date', '>=', date('Y-m-d'))->first();
    ...
    public function bank_details()
    {
        ...
        return view('doctor.doctor.doctor_bank',compact('doctor'));
    }
    ...
    public function update_doctor_profile(Request $request)
    {
        ...
        $doctor->update($data);
        $this->changeLanguage();
        return redirect('/doctor_home')->withStatus(__('Therapist updated successfully..!!'));
    }



    public function update_doctor_bank(Request $request)
    {
        $id = Doctor::where('user_id',auth()->user()->id)->first()->id;
        ...
        return redirect('/bank_details')->withStatus(__('Bank Details updated successfully..!!'));
    }
       ...
       if(auth()->user()->hasRole('super admin'))
        {
            ...
            $reviews = Review::with('appointment:id,appointment_id','user:id,name')->get();

        }else{
            $doctor = Doctor::where('user_id',auth()->user()->id)->first();
            $reviews = Review::with('appointment:id,appointment_id','user:id,name')->where('doctor_id',$doctor->id)->where('status','approved')->get();
        }
    ...
    {   
        ...
        return redirect('/doctor_review')->withStatus(__('Deleted successfully..!!'));
    }

    public function update_review(Request $request)
    {
        Review::where('id',$request->id)->update(['status'=>'approved']);
        return redirect('/doctor_review')->withStatus(__('Updated successfully..!!'));
    }
}
