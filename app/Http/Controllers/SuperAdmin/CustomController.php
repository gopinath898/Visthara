...
class CustomController extends Controller
{
    ...
    {
        $workingHours = WorkingHour::where('doctor_id',$doctor_id)->get();
        $master = [];
                ...
                if($hours->status == 1)
                {
                    foreach (json_decode($hours->period_list) as $value)
                    {
                        ...
                        else
                        {
                            ...
                            // print_r($time);die();
                            $appointment = AppointmentSessions::where([['doctor_id', $doctor_id], ['session_timeslot', $time], ['session_date', $date]])->first();
    ...
    public function cancel_max_order()
    {
        ...
        $cancel_orders = AppointmentSessions::where([['session_date', '<=', $formatted],['session_status','booked']])->get();
        foreach ($cancel_orders as $cancel_order)
        {
            $cancel_order->session_status = 'cancel';
            $cancel_order->save();
        }
        return true;
    }
