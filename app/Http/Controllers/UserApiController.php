...
class UserApiController extends Controller
{
    public function apiLogin(Request $request)
    {
        ...
        if (Auth::attempt($user))
        {
            ...
            {
                return response()->json(['success' => false, 'msg' => 'Only Client Can login'], 401);
            }
        }
        else
        {
    ...
    public function apiRegister(Request $request)
    {
        ...
        if($user->verify == 1)
        {
            ...
            {
                $user['token'] = $user->createToken('Therapist')->accessToken;
    ...
    public function apiSingleDoctor(Request $request,$id)
    {
        ...
        $doctor['reviews'] = Review::with('user:id,name,image')->where('status','approved')->where('doctor_id',$id)->get();
    ...
    public function apiBooking(Request $request)
    {
        ...
        $doctor_notification['user_type'] = 'Therapist';
    ...
    {
        $setting = Setting::first(['cod','doctor_app_id','paypal','stripe','razor','flutterwave','paystack','stripe_public_key','stripe_secret_key','paypal_sandbox_key','paypal_producation_key','razor_key','flutterwave_encryption_key','patient_app_id','playstore','appstore','privacy_policy','about_us','agora_app_id','agora_app_certificate','cancel_reason','flutterwave_key','paystack_public_key','currency_symbol','currency_code','isLiveKey','paypal_client_id','paypal_secret_key'])->makeHidden(['companyWhite','logo','favicon']);
        if (!auth('api')->check()) 
        {
            $setting = $setting->makeHidden(['stripe_public_key','stripe_secret_key','paypal_sandbox_key','paypal_producation_key','razor_key','flutterwave_encryption_key','agora_app_id','agora_app_certificate','flutterwave_key','paystack_public_key','paypal_client_id','paypal_secret_key']);
        }
