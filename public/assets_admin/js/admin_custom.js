...
$(document).ready(function()
{
    ...
    $('#treatment_id_old').on('change', function()
    {
        $.ajax({
            headers:
            {
    ...
    // Add More Hours
    $(".hours-info, .create-hours-info").on('click','.trash', function () {
    ...
    $(".create-hours").on('click', function ()
    {
        ...
        $(".create-hours-info").append(hourscontent);
        timeslot_generator();
        return false;
    });
...
function addCharge(){
    $('.delivery_charge_table').append(
        '<tr><td><input type="text" required name="min_value[]" class="form-control"></td>'+
        '<td><textarea  name="max_value[]"  class="form-control"></textarea></td>' +
        '<td ><input type="text" required name="charges[]" class="form-control"></td>' +
...
// License Add More
$(".license-info").on('click','.trash', function () {
    $(this).closest('.license-cont').remove();
    // $(this).closest('div').remove();
    return false;
});

$(".add-license").on('click', function () {
    var licensecontent = '<div class="row form-row license-cont">' +
        '<div class="col-12 col-md-10 col-lg-11">' +
            '<div class="row form-row">' +
                '<div class="col-12 col-md-6 col-lg-4">' +
                    '<div class="form-group">' +
    ...
    $(".license-info").append(licensecontent);
    return false;
});
...
$(".add-experience").on('click', function () {
    var licensecontent = '<div class="row form-row experience-cont">' +
        '<div class="col-12 col-md-10 col-lg-11">' +
            '<div class="row form-row">' +
                '<div class="col-12 col-md-6 col-lg-4">' +
                    '<div class="form-group">' +
                        '<label>Experience (in years)</label>' +
                        '<input type="number" min="1" name="experience_years[]" class="form-control">' +
                    '</div>' +
                '</div>' +
                '<div class="col-12 col-md-6 col-lg-4">' +
                    '<div class="form-group">' +
    ...
    $(".experience-info").append(licensecontent);
    return false;
});
...
function display_timeslot(date,doctor_id)
{
    console.log("dfghj");
    $.ajax({
        headers:
        ...
        type: "GET",
        url: base_url + '/display_timeslot'+'/'+doctor_id+'/'+date,
        success: function (result)
        {
...
// Display Appointment
function show_appointment(appointment_id, session_id=null) {
    $.ajax({
        headers:
        {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        success: function (result)
        {
            ...
            $('.illness').text(result.data.illness_information);
            ...
            $('.guardian_phone').text(result.data.guardian_phone);
            $('.illness_exists').text(result.data.illness_exists);
            $('.illness_information_taken').text(result.data.illness_information_taken);
...
function seeData(showClass, hideClass) 
{
    $(showClass).show();
    $(hideClass).hide();
}
...
$('#booking_date').change(function(){
    var id = $('input[name=working_doctor_id]').val();
    ...
    $.ajax({
        ...
        type: "GET",
        url: base_url + '/display_timeslot'+'/'+id+'/'+date,
        success: function (result)
        {
            $('.create_working_hours').attr('action', base_url+'/create_timeslot');
            if(result.data){
                $('input[name=working_id]').val(result.data.id);
                if(result.data.status == 1)
                {
...
function appointId(id, doctor_name='')
{
...
function setPackageDetails(div){
    $.ajax({
        headers:
        {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        type: "GET",
        url: base_url + '/getPackageDetails/'+$(div).val(),
        success: function (result) {
            $('input[name=package_price]').val(result.price);
            $('input[name=discount_price]').val(result.discountPrice);
            $('input[name=amount]').val(result.finalAmount);
        },
        error: function (err) {
        }
    });
