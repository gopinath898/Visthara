                ...
                <form action="{{ url('blog/'.$blog->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-6 form-group">
                                <label class="col-form-group">{{__('Specializations')}}</label>
                                <br>
                                <select name="category_id[]" id="category_id" class="select2 form-control @error('category_id') is-invalid @enderror" required multiple="multiple">
                                    ...
                                    <option value="{{ $category->id }}" {{ in_array($category->id,json_decode($blog->category_id ?? '') ?? []) ? 'selected' : '' }}>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                                @error('category_id')
                                    <div class="invalid-feedback">
