...
@section('title',__('All Therapist'))
@section('content')
...
<section class="section">
    ...
    <div class="card">
            <div class="card-header w-100 text-right d-flex justify-content-between">
                ...
                @can('doctor_add')
                    <a href="{{ url('therapy/create') }}">{{ __('Add New') }}</a>
                @endcan
            </div>
            <div class="card-body">
                        ...
                        <tbody>
                            @foreach ($doctors as $doctor)
                                <tr>
                                    ...
                                    @if($activePage == 'doctor')
                                        ...
                                        <td>
                                            @if (Gate::check('doctor_edit') || Gate::check('doctor_delete'))
                                                ...
                                                @can('doctor_edit')
                                                <a class="text-success" href="{{url('therapy/'.$doctor->id.'/edit')}}">
                                    ...
                                    @elseif($activePage == 'schedule')
                                        <td>
                                            <a href="{{ url('schedule/'.'?doctor_id='.$doctor->user_id) }}" class="btn btn-primary btn-sm">
                                                Schedule Timings
                                            </a>
                                        </td>
                                    @endif
