...
<section class="section">
    @include('layout.breadcrumb',[
        'title' => __('Therapist Schedule Timing'),
        'url' => url('doctor'),
                            ...
                            <div class="schedule-nav">
                                <ul class="nav nav-tabs nav-justified">
                                    @foreach ($doctor->workingHours as $working)
                                        <li class="nav-item">
                                            <a class="nav-link {{ $loop->iteration == 1 ? 'active' : '' }}" onclick="display_timeslot({{ $working->id }})" data-toggle="tab" href="#slot_sunday">{{ $working->booking_date }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
