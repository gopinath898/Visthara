    ...
    <div class="section_body">
        ...
        <div class="card">
            <div class="card-header w-100 text-right d-flex justify-content-between">
                ...
                @can('lab_add')
                    <a href="{{  url('faq_category') }}">{{ __('View FAQ Categories') }}</a>                
                    <a href="{{  url('faq_data/create') }}">{{ __('Add New') }}</a>                
                @endcan
            </div>
            <div class="card-body">
                        ...
                        <tbody>
                            @foreach ($radiologyCategories as $radiologyCategory)
                                <tr>
                                    ...
                                    <td>{{ $radiologyCategory->sort }}</td>
                                    <td>{{ $radiologyCategory->name }}</td>
                                    <td>{{ $radiologyCategory->category->name ?? ''}}</td>
                                    <td>
                                    ...
                                    <td>
                                        @if (Gate::check('radiology_category_edit') || Gate::check('radiology_category_delete'))
                                            ...
                                            <a class="text-success" href="{{url('faq_data/'.$radiologyCategory->id.'/edit')}}">
                                                <i class="far fa-edit"></i>
                                            </a>
                                            @endcan
                                            @can('radiology_category_delete')
                                            <a class="text-danger" href="javascript:void(0);" onclick="deleteData('faq_data',{{ $radiologyCategory->id }})">
