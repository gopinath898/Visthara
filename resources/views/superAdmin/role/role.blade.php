                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if (Gate::check('role_edit') || Gate::check('role_delete'))
                                            @can('role_edit')
                                                @if ($role->default == 0)
                                                    <a class="text-success {{ $role->name == 'super admin' ? 'disabled' : ''  }} {{ $role->name == 'pharmacy' ? 'disabled' : ''  }} {{ $role->name == 'Therapist' ? 'disabled' : ''  }}" href="{{url('role/'.$role->id.'/edit/')}}">
                                            ...
                                            @can('role_delete')
                                                @if ($role->default == 0)
                                                    <a href="javascript:void(0)"  class="text-danger {{ $role->name == 'super admin' ? 'disabled' : ''  }} {{ $role->name == 'pharmacy' ? 'disabled' : ''  }} {{ $role->name == 'Therapist' ? 'disabled' : ''  }}" onclick="deleteData('role',{{ $role->id }})">
